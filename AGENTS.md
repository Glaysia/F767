...
- HAL ADC/TIM modules enabled. ADC1 (IN0) and ADC3 (IN6) set to 8-bit, single-channel, DMA circular, EOC on sequence, sampling time 3 cycles, external trigger edge rising on TIM8 TRGO.
- TIM8: PSC=0, ARR=59 (~trigger frequency per IOC), TRGO=Update; `AdcHandler::StartDma` starts ADC1/ADC3 DMAs and `HAL_TIM_Base_Start` on TIM8.
- TIM5 still configured (PSC=0, ARR=85, TRGO=Update, preload enabled) but not driving ADCs. TIM3 no longer used for ADC triggers.
...
- Decide whether to keep TIM8 TRGO or move to TIM5 TRGO; if switching, change ADC external trigger and start the matching timer.
